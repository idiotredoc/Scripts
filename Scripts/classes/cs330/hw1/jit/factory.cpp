...
Factory::Factory (const Factory& f) //copy constructor
  : numOrders(f.numOrders), maxOrders(f.maxOrders), name(f.name)
{
  orders = new Order[maxOrders];
...
const Factory& Factory::operator= (const Factory& f) //assignment
{
  numOrders = f.numOrders;
  if (f.numOrders > maxOrders)
    {
